...
```ts
  ...
  4  | 
  5  |     let r1 = await request.get(`https://parabank.parasoft.com/parabank/services/bank/customers/${getCustomerId}/accounts`);
  6  |     let body = await r1.text();
  7  |     console.log(body);
> 8  |     expect(r1.status()).toBe(200);
     |                         ^ Error: expect(received).toBe(expected) // Object.is equality
  9  |     expect(body).toContain(`<customerId>${getCustomerId}</customerId>`);
