...
- Location: tests\tests\e2e\FullAccountLifecycle.spec.ts:9:5
...
```ts
  ...
  7  | import transferData from'../../../test-data/transferFundsData.json';
  8  | 
  9  | test('Full Account Lifecycle',async({page,request})=>{
  10 | await page.goto('https://parabank.parasoft.com/parabank');
  11 | await new LoginPage(page).login(loginData.username,loginData.password);
  12 | 
> 13 | await page.getByRole('link',{name:'Open New Account'}).click();
  ...
  22 | const response=await request.get('https://parabank.parasoft.com/parabank/services/bank/customers/12212/accounts');
